"""
Query BD GMV %, KVAM GMV %, and SD Promo Code Contribution
Last 6 Months (May 2025 - October 2025) for Penang
"""
...
# KVAM names
kvam_names = ['Darren', 'Suki', 'Chia Yee', 'MGS']

print("="*80)
print("BD, KVAM, and SD PROMO CODE ANALYSIS - PENANG")
print("Last 6 Months (May 2025 - October 2025)")
print("="*80)
print(f"\nT20/T3 Merchants: {len(t20_merchant_ids)}")
print(f"SD Promo Codes: {len(sd_promo_codes)}")
print(f"KVAM Names: {', '.join(kvam_names)}")
...
-- Last 6 Months (May 2025 - October 2025)
-- Promo Codes: {', '.join(sd_promo_codes[:10])}... (total {len(sd_promo_codes)} codes)
-- ============================================================================
...
sd_promo_orders AS (
    ...
    WHERE f.city_id = 13
        ...
        AND f.date_id < 20251101
        AND f.business_type = 0
        AND COALESCE(fod.promo_code_os, '') IN ('{promo_codes_sql}')
...
# Save queries
queries = [
    ("BD GMV % Trends", bd_gmv_query),
    ("KVAM GMV % Trends", kvam_gmv_query),
    ("SD Promo Code Contribution", sd_promo_query)
]

with open('query_bd_kvam_promo_analysis.sql', 'w', encoding='utf-8') as f:
    f.write("-- ============================================================================\n")
    f.write("-- BD, KVAM, and SD PROMO CODE ANALYSIS - PENANG\n")
    f.write("-- Last 6 Months (May 2025 - October 2025)\n")
    f.write("-- ============================================================================\n\n")
    
    for i, (title, query) in enumerate(queries, 1):
        f.write(f"\n{'='*80}\n")
        f.write(f"QUESTION {i}: {title}\n")
        f.write(f"{'='*80}\n\n")
        f.write(query)
        f.write("\n\n")

print("\n[OK] Generated SQL queries saved to: query_bd_kvam_promo_analysis.sql")
print("\n[INFO] Query Summary:")
for i, (title, _) in enumerate(queries, 1):
    print(f"   {i}. {title}")

print("\n[WARNING] Next Steps:")
print("   1. Execute queries in Presto/Hubble")
print("   2. Analyze results for trends and T3/T20 impact")
