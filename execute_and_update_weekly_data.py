"""
Execute queries via MCP and update hardcoded data in process_and_send_weekly_report.py
This script is called when /weekly_report is run
"""
...
def update_hardcoded_data(oc_results, top_cities_results):
    ...
    if oc_results and len(oc_results) > 0:
        ...
        # Update OC data section - find the pattern and replace
        today_str = datetime.now().strftime('%b %d, %Y')
        
        # Build new OC data string
...
oc_data = [{{
    ...
    'same_month_last_year_earning_per_mex': {round(oc_row.get('same_month_last_year_earning_per_mex', 0), 2)}
        ...
        # Replace OC data section using regex
    ...
    if top_cities_results and len(top_cities_results) > 0:
        ...
        for city_row in top_cities_results:
            ...
            city_name = city_dict.get('city_name', '').replace("'", "\\'")
            city_data = f"    {{'city_name': '{city_name}', 'city_id': {city_dict.get('city_id', 0)}, "
...
if __name__ == '__main__':
    print("This script should be called from Cursor chat")
    print("The AI will execute queries via MCP and call update_hardcoded_data() with results")
