...
def export_merchants_to_excel():
    ...
    # Note: This script should be run in an environment where MCP tools are available
    # For now, we'll create a script that can be executed with the MCP tool
    
    print("\nTo execute this query, use the MCP tool:")
    print("mcp_mcp-grab-data_run_presto_query with the query above")
    print("\nThen save the results to Excel using pandas.")
    
    return query

if __name__ == "__main__":
    # Save query to a file for reference
    with open('merchant_query.sql', 'w') as f:
        f.write(query)
    print("Query saved to merchant_query.sql")
    print("\nPlease execute the query using the MCP tool and then process the results.")
