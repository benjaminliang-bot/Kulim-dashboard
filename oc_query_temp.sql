...
WITH this_week AS (
    ...
    WHERE f.country_id = 1 
      AND f.city_id != 1
      AND f.date_id >= 20251123 AND f.date_id <= 20251130
...
same_week_last_month AS (
    ...
    WHERE f.country_id = 1 
      AND f.city_id != 1
      AND f.date_id >= 20251023 AND f.date_id <= 20251030
      AND f.business_type = 0
),
same_week_last_year AS (
    ...
    WHERE f.country_id = 1 
      AND f.city_id != 1
      AND f.date_id >= 20241123 AND f.date_id <= 20241130
      AND f.business_type = 0
),
...
sessions_this_week AS (
    ...
    WHERE country_id = 1 
      AND city_id != 1
      AND date_id >= 20251123 AND date_id <= 20251130
      AND business = 'food'
),
sessions_same_week_last_month AS (
    ...
    WHERE country_id = 1 
      AND city_id != 1
      AND date_id >= 20251023 AND date_id <= 20251030
      AND business = 'food'
),
sessions_same_week_last_year AS (
    ...
    WHERE country_id = 1 
      AND city_id != 1
      AND date_id >= 20241123 AND date_id <= 20241130
      AND business = 'food'
),
...
new_pax_this_week AS 
    -- New Pax calculation for week 20251123 to 20251130
    (
        ...
        FROM (
            ...
            WHERE f.country_id = 1 
              AND city_id != 1
              AND f.date_id >= 20251123 
              AND f.date_id <= 20251130
              AND f.business_type = 0
              AND f.booking_state_simple = 'COMPLETED'
              AND (
                  ...
                  f.passenger_id IN (
                      ...
                      WHERE first_order_date >= 20251123
                        AND first_order_date <= 20251130
                  )
                  OR
                  ...
                  f.passenger_id IN (
                      ...
                      FROM (
                          ...
                          WHERE country_id = 1
                            ...
                            AND business_type = 0
                            AND booking_state_simple = 'COMPLETED'
                            AND date_id < 20251123
                          GROUP BY passenger_id
                      ) last_orders
                      WHERE last_order_date < 20251123 - 36500  -- More than 365 days ago
...
new_pax_last_month AS 
    -- New Pax calculation for week 20251023 to 20251030
    (
        SELECT COUNT(DISTINCT new_pax.passenger_id) as new_pax_count
        FROM (
            SELECT DISTINCT f.passenger_id
            ...
            WHERE f.country_id = 1 
              AND city_id != 1
              AND f.date_id >= 20251023 
              AND f.date_id <= 20251030
              AND f.business_type = 0
              AND f.booking_state_simple = 'COMPLETED'
              AND (
                  -- First order was within this week
                  f.passenger_id IN (
                      ...
                      WHERE first_order_date >= 20251023
                        AND first_order_date <= 20251030
                  )
                  OR
                  -- Last transaction before this week was more than a year ago
                  f.passenger_id IN (
                      ...
                      FROM (
                          ...
                          WHERE country_id = 1
                            ...
                            AND business_type = 0
                            AND booking_state_simple = 'COMPLETED'
                            AND date_id < 20251023
                          GROUP BY passenger_id
                      ) last_orders
                      WHERE last_order_date < 20251023 - 36500  -- More than 365 days ago
                        AND passenger_id NOT IN (
                            -- Exclude passengers who ordered in the last year (before this week)
                            SELECT DISTINCT passenger_id
                            FROM ocd_adw.f_food_metrics
...
new_pax_last_year AS 
    -- New Pax calculation for week 20241123 to 20241130
    (
        SELECT COUNT(DISTINCT new_pax.passenger_id) as new_pax_count
        FROM (
            SELECT DISTINCT f.passenger_id
            ...
            WHERE f.country_id = 1 
              AND city_id != 1
              AND f.date_id >= 20241123 
              AND f.date_id <= 20241130
              AND f.business_type = 0
              AND f.booking_state_simple = 'COMPLETED'
              AND (
                  -- First order was within this week
                  f.passenger_id IN (
                      ...
                      WHERE first_order_date >= 20241123
                        AND first_order_date <= 20241130
                  )
                  OR
                  -- Last transaction before this week was more than a year ago
                  f.passenger_id IN (
                      ...
                      FROM (
                          ...
                          WHERE country_id = 1
                            ...
                            AND business_type = 0
                            AND booking_state_simple = 'COMPLETED'
                            AND date_id < 20241123
                          GROUP BY passenger_id
                      ) last_orders
                      WHERE last_order_date < 20241123 - 36500  -- More than 365 days ago
                        AND passenger_id NOT IN (
                            -- Exclude passengers who ordered in the last year (before this week)
                            SELECT DISTINCT passenger_id
                            FROM ocd_adw.f_food_metrics
