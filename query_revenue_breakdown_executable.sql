-- ============================================================================
-- REVENUE BREAKDOWN BY INDIVIDUAL (AM & MGS) - EXECUTABLE QUERIES
-- Last 6 Months (May 2025 - October 2025)
-- ============================================================================
-- Revenue Sources:
--   1. Commission Revenue: Gross billing from commission (deliveries/dine out)
--   2. Ads/Spotlight Revenue: Revenue from ads or spotlight
--   3. MEX Funded Campaign: MEX-funded promotional campaign spending
-- ============================================================================


-- ============================================================================
-- QUERY 1: Revenue Breakdown: Darren (AM)
-- ============================================================================
...
-- Last 6 Months (May 2025 - October 2025)
-- Filtered by: am_name = 'darren.ng@grabtaxi.com'
-- ============================================================================

WITH darren_merchants AS (
    SELECT DISTINCT merchant_id_nk
    FROM ocd_adw.d_merchant
    WHERE city_id = 13
        AND am_name = 'darren.ng@grabtaxi.com'
),
...
commission_revenue AS (
    ...
    INNER JOIN darren_merchants m ON f.merchant_id = m.merchant_id_nk
    WHERE f.city_id = 13
...
ads_revenue AS (
    ...
    INNER JOIN darren_merchants m ON a.merchant_id = m.merchant_id_nk
...
mex_campaign_spend AS (
    ...
    INNER JOIN darren_merchants m ON d.merchant_id = m.merchant_id_nk
    WHERE d.city_id = 13
...
-- ============================================================================
-- QUERY 2: Revenue Breakdown: Suki (AM)
-- ============================================================================
...
-- Last 6 Months (May 2025 - October 2025)
-- Filtered by: am_name = 'suki.teoh@grabtaxi.com'
-- ============================================================================

WITH suki_merchants AS (
    SELECT DISTINCT merchant_id_nk
    FROM ocd_adw.d_merchant
    WHERE city_id = 13
        AND am_name = 'suki.teoh@grabtaxi.com'
),
-- 1. COMMISSION REVENUE (Gross billing from deliveries/dine out)
commission_revenue AS (
    SELECT 
        ...
        END) as active_merchants
    FROM ocd_adw.f_food_metrics f
    INNER JOIN suki_merchants m ON f.merchant_id = m.merchant_id_nk
    WHERE f.city_id = 13
        AND f.business_type = 0
        AND f.date_id >= 20250501
        AND f.date_id < 20251101
...
ads_revenue AS (
    SELECT 
        ...
        COUNT(DISTINCT merchant_id) as merchants_in_ads
    FROM ocd_adw.agg_ads_merchant a
    INNER JOIN suki_merchants m ON a.merchant_id = m.merchant_id_nk
    WHERE date_id >= 20250501
        AND date_id < 20251101
    GROUP BY CAST(SUBSTRING(CAST(date_id AS VARCHAR), 1, 6) AS INTEGER)
),
...
mex_campaign_spend AS (
    SELECT 
        ...
        COUNT(DISTINCT mfc_campaign_id) as unique_campaigns
    FROM ocd_adw.f_food_discount d
    INNER JOIN suki_merchants m ON d.merchant_id = m.merchant_id_nk
    WHERE d.city_id = 13
        AND d.date_id >= 20250501
        AND d.date_id < 20251101
        AND d.booking_state_simple = 'COMPLETED'
...
-- ============================================================================
-- QUERY 3: Revenue Breakdown: Chia Yee (AM)
-- ============================================================================
...
-- Last 6 Months (May 2025 - October 2025)
-- Filtered by: am_name = 'chiayee.leong@grabtaxi.com'
-- ============================================================================

WITH chia_yee_merchants AS (
    SELECT DISTINCT merchant_id_nk
    FROM ocd_adw.d_merchant
    WHERE city_id = 13
        AND am_name = 'chiayee.leong@grabtaxi.com'
),
-- 1. COMMISSION REVENUE (Gross billing from deliveries/dine out)
commission_revenue AS (
    SELECT 
        ...
        END) as active_merchants
    FROM ocd_adw.f_food_metrics f
    INNER JOIN chia_yee_merchants m ON f.merchant_id = m.merchant_id_nk
    WHERE f.city_id = 13
        AND f.business_type = 0
        AND f.date_id >= 20250501
        AND f.date_id < 20251101
...
ads_revenue AS (
    SELECT 
        ...
        COUNT(DISTINCT merchant_id) as merchants_in_ads
    FROM ocd_adw.agg_ads_merchant a
    INNER JOIN chia_yee_merchants m ON a.merchant_id = m.merchant_id_nk
    WHERE date_id >= 20250501
        AND date_id < 20251101
    GROUP BY CAST(SUBSTRING(CAST(date_id AS VARCHAR), 1, 6) AS INTEGER)
),
...
mex_campaign_spend AS (
    SELECT 
        ...
        COUNT(DISTINCT mfc_campaign_id) as unique_campaigns
    FROM ocd_adw.f_food_discount d
    INNER JOIN chia_yee_merchants m ON d.merchant_id = m.merchant_id_nk
    WHERE d.city_id = 13
        AND d.date_id >= 20250501
        AND d.date_id < 20251101
        AND d.booking_state_simple = 'COMPLETED'
...
-- ============================================================================
-- QUERY 4: Revenue Breakdown: Teoh Jun Ling (MGS)
-- ============================================================================
...
-- ============================================================================
-- QUERY 5: Revenue Breakdown: Hon Yi Ni (MGS)
-- ============================================================================
...
-- ============================================================================
-- QUERY 6: Revenue Breakdown: Low Jia Ying (MGS)
-- ============================================================================
...
-- ============================================================================
-- QUERY 7: Revenue Breakdown: Lee Sook Chin (MGS)
-- ============================================================================
