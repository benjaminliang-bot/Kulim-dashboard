"""Send weekly report with live MCP data - direct approach"""
import requests
import json
...
with open("process_and_send_weekly_report.py", "r", encoding="utf-8") as f:
    code = f.read()
    # Skip encoding wrapper to avoid issues
...
# Live data from MCP queries (executed above)
oc_data = [{
...
if __name__ == '__main__':
    ...
    # Write result to file to avoid stdout issues
    with open("report_send_result.txt", "w") as f:
        if success:
            f.write("SUCCESS: Weekly report sent to Slack!\n")
        else:
            f.write(f"Response: {response.status_code} - {response.text[:200]}\n")
    
    # Also try to output (may fail due to encoding)
    try:
        if success:
