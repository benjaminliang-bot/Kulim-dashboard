...
Queries all merchants analyzed in the growth analysis and exports to Excel format
"""

import pandas as pd
from mcp_mcp_grab_data_run_presto_query import run_presto_query

# SQL Query to get all Mainland merchants (matching Query 1 from the analysis)
query = """
...
def export_merchants_to_excel():
    ...
    if isinstance(result, dict) and 'data' in result:
        ...
        # Try to parse as DataFrame directly
        df = pd.DataFrame(result)
    
    print(f"Found {len(df)} merchants")
    
    # Export to Excel
    output_file = "penang_mainland_merchants_analyzed.xlsx"
    df.to_excel(output_file, index=False, engine='openpyxl')
    
    print(f"✅ Exported {len(df)} merchants to {output_file}")
    print(f"\nColumns exported:")
    for col in df.columns:
        print(f"  - {col}")
    
    print(f"\nSummary by Status:")
    if 'merchant_status' in df.columns:
        print(df['merchant_status'].value_counts())
    
    print(f"\nSummary by Halal Status:")
    if 'halal_status' in df.columns:
        print(df['halal_status'].value_counts())
